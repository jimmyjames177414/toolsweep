...
- **Validity** — the result loads and dumps through every adapter, has unique tool names,
  and never produces a name a target protocol would reject.
...
- **Do not invent semantics.** If the catalogue does not say something, the honest move is
  for the level to be *inert* — toolsweep reports that as "nothing to measure here" rather
  than as a null result. `params.required=minimal_required` works exactly this way.
...
- **No invented numbers.** Every number in a README or a docstring must be reproducible by
  a command, or explicitly labelled synthetic.
...
a stub — it goes in the README's "Not built yet" list and becomes an issue. A stub that
looks implemented is worse than an admitted gap.
